export class BundleUtils {
    ...
    getResources<T = any>(bundleEntry: any[] | null | undefined, resourceTypeToFilter: string): T[] {
        return bundleEntry?.length ? bundleEntry.filter(x => x['resource']['resourceType'] === resourceTypeToFilter).map(x => x['resource']) : [];
    }
     ...
     */
    getResource<T = any>(bundleEntry: any[] | null | undefined, resourceId: string): T | undefined | null {
     ...
     * @returns single resource, undefined if not found, or null if input is null
     */
    getResourceByFullUrl<T = any>(bundleEntry: any[] | null | undefined, fullUrl: string): T | undefined | null {
        if (!bundleEntry) return null;
